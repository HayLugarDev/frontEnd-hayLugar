      ...
      <li v-for="(reservation, index) in reservations" :key="index"
        ...
        <div class="flex flex-col xl:grid xl:grid-cols-12 gap-3 text-gray-700 font-medium">
          <div class="col-span-12 flex flex-wrap items-center gap-3 justify-between">
            <div class="flex items-center gap-3">
              ...
              <span class="pill" :class="statusColors[reservation.status] || 'bg-gray-200 text-gray-700'">
              ...
              <span v-else-if="reservation.captured_amount_cents > 0"
                class="pill bg-primary/10 text-primary border-primary/20">
                Pago inmediato
              </span>
            </div>
          ...
          <div class="col-span-6 flex items-start gap-3">
            ...
            <div>
              ...
              <div class="text-xs text-gray-500">Check-in: {{ formatDate(reservation.start_time) }} · Check-out: {{
                formatDate(reservation.end_time) }}</div>
            </div>
          </div>
        ...
        <div v-if="reservation.status === 'in_progress'">
          ...
          <div class="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
            <div class="h-2 bg-primary transition-all" :style="{ width: (progress[reservation.id] || 0) + '%' }"></div>
          </div>
        </div>
        ...
        <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mt-2">
          <div class="flex flex-wrap gap-2">
            ...
            <template v-else-if="reservation.status === 'verified' || reservation.status === 'in_progress'">
              ...
              <button v-if="quote && selectedReservation?.id === reservation.id && quote.remainder_cents > 0"
                class="btn-amber" :disabled="extra.show"
                @click="openExtraChargeModal(quote.remainder_cents, reservation)">
                <font-awesome-icon :icon="['fas', 'plus-circle']" />
                Cobrar diferencia {{ formatCents(quote.remainder_cents) }}
              </button>

              <button class="btn-gray-outline" v-if="reservation.status !== 'in_progress'"
          ...
          <div v-if="quote && selectedReservation?.id === reservation.id"
            ...
            <div class="text-right text-red-600" v-if="quote.penalty_cents > 0">{{ formatCents(quote.penalty_cents) }}
            </div>
            <div class="text-gray-500" v-if="quote.remainder_cents > 0">Falta cobrar</div>
            <div class="text-right text-amber-600" v-if="quote.remainder_cents > 0">{{
              formatCents(quote.remainder_cents) }}</div>
          </div>
        </div>
      </li>
    </ul>
...
/* Buttons */
.btn-green {
  @apply bg-green-600 text-white px-4 py-2 rounded-lg shadow hover:shadow-md transition-all flex items-center gap-2;
}

.btn-red {
  @apply bg-red-500 text-white px-4 py-2 rounded-lg shadow hover:shadow-md transition-all flex items-center gap-2;
}

.btn-indigo {
  @apply bg-indigo-600 text-white px-4 py-2 rounded-lg shadow hover:shadow-md transition-all flex items-center gap-2;
}

.btn-primary {
  @apply bg-primary text-white px-4 py-2 rounded-lg shadow hover:shadow-md transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed;
}

.btn-amber {
  @apply bg-amber-500 text-white px-4 py-2 rounded-lg shadow hover:shadow-md transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed;
}

.btn-gray {
  @apply px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 text-gray-800;
}

.btn-gray-outline {
  @apply px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50;
}
...
.input {
  @apply mt-1 w-full border rounded-lg px-3 py-2;
}
