  ...
  <header
    ...
    <div v-if="authChecked" class="flex flex-row justify-between gap-2">
      ...
      <div v-if="route.path === '/dashboard' || route.path.startsWith('/espacio')" class="relative flex flex-row sm:gap-2 items-center max-h-12">
        <button v-if="routeConfig.showPublicarButton" @click="verifyToken('/add-space')"
        ...
        <ul v-if="openMenu"
          ...
          <li v-if="userStore.user" @click="verifyToken('/wallet')" class="text-gray-700 px-4 py-2 w-full hover:bg-gray-100">
            Wallet</li>
