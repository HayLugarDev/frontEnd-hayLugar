            ...
            <form @submit.prevent="login" class="space-y-6">
              ...
              <div class="relative">
                <input
                  ...
                  class="peer w-full px-4 pt-5 pb-2 border border-gray-300 rounded-md text-base placeholder-transparent focus:outline-none focus:ring-2 focus:ring-accent transition-all duration-200 disabled:opacity-60"
                />
                <label
                  :class="[
              ...
              <div class="relative">
                <input
                  ...
                  :disabled="cargando"
                  @focus="isFocused.password.value = true"
              ...
              <button
                type="submit"
                :disabled="cargando"
                class="flex justify-center w-full bg-accent text-dark py-2 rounded-md disabled:opacity-60"
              >
...
// soporte visual de los labels flotantes
const isFocused = {
  email: ref(false),
  password: ref(false),
}
...
const login = async () => {
  ...
  try {
    ...
    if (response.data?.token) {
      // guarda token para /api/terms/accept
      localStorage.setItem('token', response.data.token)

      // setea user en tu store
      const userStore = useUserStore()
      userStore.setUser(response.data.user)

      // si debe re-aceptar términos -> forzar flujo
      if (response.data?.terms?.mustReaccept) {
        router.push('/aceptar-terminos')
        return
      }

      // si venías redirigido desde una ruta protegida, volvé allí
