  ...
  <div class="min-h-screen bg-gradient-to-br from-[#0D1B2A] via-[#1B263B] to-[#0D1B2A] text-white overflow-hidden">
    ...
    <header class="relative z-10 px-6 pt-16 py-4 md:py-8 md:px-12 flex items-center justify-between md:hidden">
      ...
      <button
        ...
        <i class="fa-solid fa-industry text-newgreen"></i>
    ...
    <section
      ...
      <div class="flex items-center gap-3">
        <label class="flex items-center gap-2 text-sm text-[#B0BEC5]">
          <input v-model="filters.security" type="checkbox" class="accent-newgreen" />
          Seguridad
        </label>
        <label class="flex items-center gap-2 text-sm text-[#B0BEC5]">
          <input v-model="filters.loadingDock" type="checkbox" class="accent-newgreen" />
          Muelle
        </label>
      </div>
      ...
      <div class="flex items-center gap-3 mt-2 md:mt-0">
        <button @click="fetchSpaces"
          class="bg-[#00B4D8] hover:bg-newgreen text-[#0D1B2A] font-semibold rounded-xl px-5 py-2 transition-all">
          Buscar
        </button>
      ...
      <div class="rounded-2xl overflow-hidden shadow-lg border border-white/10 h-[600px]">
        <CustomGoogleMap :center="mapCenter" :zoom="13" :options="{ disableDefaultUI: true, zoomControl: true }">
          <GMapMarker v-for="space in spaces" :key="space.id"
            :position="{ lat: Number(space.latitude), lng: Number(space.longitude) }" :options="{
              title: space.name,
              icon: {
                url: '/assets/logo.png',
