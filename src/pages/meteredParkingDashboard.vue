  ...
  <div class="min-h-screen bg-secondary">
    ...
    <div class="px-6 pt-20 md:pt-4 flex items-center gap-3">
      ...
      <div class="ml-auto flex items-center gap-2">
        <button class="px-3 py-1.5 rounded-full text-sm font-medium border bg-white hover:bg-gray-50 transition"
          :class="showMap ? 'text-primary border-primary/30' : 'text-gray-700 border-gray-300'" @click="showMap = true">
          Mapa
        </button>
        <button class="px-3 py-1.5 rounded-full text-sm font-medium border bg-white hover:bg-gray-50 transition"
    ...
    <div class="px-4 mt-3 flex flex-wrap gap-2 items-center">
      <button class="px-3 py-1.5 rounded-full text-sm font-semibold transition border"
    ...
    <div class="p-4">
      ...
      <div v-if="showMap" class="w-full h-[70vh] relative rounded-xl overflow-hidden shadow">
        <CustomGoogleMap :center="center" :zoom="zoom" :options="{ ...mapOptions, styles: streetTechStyle }"
          :debug="true" :locateUser="true" />



        <!-- Tooltip flotante -->
        <transition name="fade">
          <div v-if="hovered" class="absolute left-4 top-4 glass p-3 text-xs text-gray-800 max-w-[340px]">
            <div class="flex items-center gap-2">
              ...
              <div class="font-semibold">
                {{ hovered.label }}
                <span v-if="activeSessionsMap[hovered.id]"
            ...
            <button
              ...
              :disabled="hovered.status === 'full'" @click="openMetered(hovered)">
              Iniciar estacionamiento
            </button>

            <button v-if="activeSessionsMap[hovered.id]"
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        <div v-for="blk in snappedBlocksFiltered" :key="'card-' + (blk.id ?? blk.label)"
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="text-sm text-gray-800 font-medium">
              {{ blk.label }}
              <span v-if="activeSessionsMap[blk.id]"
          ...
          <button v-if="activeSessionsMap[blk.id]"
            class="mt-2 w-full border border-rose-300 text-rose-700 font-semibold rounded-md py-1.5 hover:bg-rose-50 transition"
...
const polylineOptions = (status, clickable = false) => {
  const base = {
    strokeOpacity: 0.95, strokeWeight: 6, clickable,
